...
/*   Updated: 2026/08/21 16:44:24 by quill            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*parse_texture_path(char *line)
{
	...
	if(access(line + j, F_OK) != 0)
		error_message("Texture file does not exist");
	path = ft_strdup(line + j);
	if (!path)
		error_message("Failed to allocate memory for texture path");
	return (path);
}
...
int    parse_texture(t_framework *fw, char *line)
{
	if(ft_strncmp(line, "NO ", 3) == 0)
	{
		if (fw->game.images.no_path)
			error_message("North texture already set");
		fw->game.images.no_path = parse_texture_path(line + 3);
	}
	else if(ft_strncmp(line, "SO ", 3) == 0)
	{
		if (fw->game.images.so_path)
			error_message("South texture already set");
		fw->game.images.so_path = parse_texture_path(line + 3);
	}
	else if(ft_strncmp(line, "WE ", 3) == 0)
	{
		if (fw->game.images.we_path)
			error_message("West texture already set");
		fw->game.images.we_path = parse_texture_path(line + 3);
	}
	else if(ft_strncmp(line, "EA ", 3) == 0)
	{
		if (fw->game.images.ea_path)
			error_message("East texture already set");
		fw->game.images.ea_path = parse_texture_path(line + 3);
	}
	else
		return (1);
	return (0);
...
void	parse_color(t_color *color, char *line)
{
	...
	i = 0;
	if (color->set)
		error_message("Color already set");
	color->set = 1;
	while(line[i] == ' ')
		i++;
...
void	check_config_complete(t_framework *fw)
{
	if(!fw->game.images.no_path || !fw->game.images.so_path 
		...
		error_message("Missing configuration information");
}
...
void	parse_config(t_framework *fw)
{
	...
	while(fw->config[i])
	{
		if (ft_strncmp(fw->config[i], "F ", 2) == 0)
			parse_color(&fw->game.floor, fw->config[i]);
		else if (ft_strncmp(fw->config[i], "C ", 2) == 0)
			parse_color(&fw->game.ceiling, fw->config[i]);
		else if (parse_texture(fw, fw->config[i]) != 0)
			error_message("Invalid configuration line");
		i++;
	}
